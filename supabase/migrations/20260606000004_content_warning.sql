-- Content warning field on posts
alter table posts
  add column if not exists content_warning text
  constraint posts_content_warning_length check (char_length(content_warning) <= 100);

-- Update home feed RPC to include content_warning
-- Must drop first because return type changes (adding content_warning column)
